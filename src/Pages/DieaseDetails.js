...
function DieaseDetails() {
  ...
  const DeleteDiease = async () => {
    const res = await SubmitResponse(`desiesdelete?desies_id=${id}`);
    if (res?.status == 200) {
      navigate(-1);
    }
  };
            ...
            <Col md={6}>
              <div className="dieases-detail">
                <div className="heading-details">
                  ...
                  <div>
                    {/* <span className="mx-2">
                      <Link to={`/EditDieases/${id}`}>
                        <img src={Edit} />{" "}
                      </Link>
                    </span> */}
                    <span className="mx-2" onClick={() => DeleteDiease()}>
                      <img src={Delete} />{" "}
                    </span>
                    <span>
